    ...
    public static class ControllerCreator
    {
        ...
        private static bool Cria(MD_Tabela tabela, string nomeProjeto)
        {
            ...
            try
            {
                ...
                stringBuilder.AppendLine($"");
                stringBuilder.AppendLine($"        /// <summary>");
                stringBuilder.AppendLine($"        /// Update status");
                stringBuilder.AppendLine($"        /// </summary>");
                stringBuilder.AppendLine($"        /// <param name=\"status\"></param>");
                stringBuilder.AppendLine($"        /// <param name=\"id\"></param>");
                stringBuilder.AppendLine($"        /// <returns><![CDATA[Task<IActionResult>]]></returns>");
                stringBuilder.AppendLine($"        [HttpPut(\"UpdateStatus\")]");
                stringBuilder.AppendLine($"        public async Task<IActionResult> UpdateStatus(Status status, long id)");
                stringBuilder.AppendLine($"        {{");
                stringBuilder.AppendLine($"            var result = await _mainAppService.UpdateStatus(status, id);");
                stringBuilder.AppendLine($"");
                stringBuilder.AppendLine($"            return Ok(result);");
                stringBuilder.AppendLine($"        }}");
                stringBuilder.AppendLine($"");

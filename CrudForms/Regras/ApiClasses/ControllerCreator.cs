    ...
    public static class ControllerCreator
    {
        ...
        private static bool Cria(MD_Tabela tabela, string nomeProjeto)
        {
            ...
            try
            {
                ...
                stringBuilder.AppendLine($"        public async Task<IActionResult> Get(Guid code, string? include = null)");
                stringBuilder.AppendLine($"        {{");
                ...
                stringBuilder.AppendLine($"");
                stringBuilder.AppendLine($"        /// <summary>");
                stringBuilder.AppendLine($"        /// Change status");
                stringBuilder.AppendLine($"        /// </summary>");
                stringBuilder.AppendLine($"        /// <param name=\"code\"></param>");
                stringBuilder.AppendLine($"        /// <param name=\"isActive\"></param>");
                stringBuilder.AppendLine($"        /// <param name=\"isDeleted\"></param>");
                stringBuilder.AppendLine($"        /// <returns><![CDATA[Task<IActionResult>]]></returns>");
                stringBuilder.AppendLine($"        [HttpPut(\"{{code}}/ChangeStatus\")]");
                stringBuilder.AppendLine($"        public async Task<IActionResult> ChangeStatus(Guid code, bool isActive, bool isDeleted = false)");
                stringBuilder.AppendLine($"        {{");
                stringBuilder.AppendLine($"            var result = await _mainAppService.ChangeStatus(code, isActive, isDeleted);");
                stringBuilder.AppendLine($"");
                stringBuilder.AppendLine($"            return Ok(result);");
                stringBuilder.AppendLine($"        }}");
                stringBuilder.AppendLine($"");

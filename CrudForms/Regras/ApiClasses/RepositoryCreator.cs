    ...
    public static class RepositoryCreator
    {
        ...
        private static bool CriaImplementacao(MD_Tabela tabela, string nomeProjeto)
        {
            ...
            try
            {
                ...
                stringBuilder.AppendLine($"        {{");
                stringBuilder.AppendLine($"            return await GetAllAsync(s => s.IsDeleted.Equals(false), include, orderBy: \"Created: Desc\");");
                stringBuilder.AppendLine($"        }}");
                stringBuilder.AppendLine($"");
                stringBuilder.AppendLine($"        public async Task<IEnumerable<Main>> GetAllAsync(Guid parentCode, string[] include = null)");
                stringBuilder.AppendLine($"        {{");
                stringBuilder.AppendLine($"            return await GetAllAsync(s => s.IsDeleted.Equals(false), include, orderBy: \"Created: Desc\");");
                stringBuilder.AppendLine($"        }}");
                stringBuilder.AppendLine($"");
                stringBuilder.AppendLine($"        public async Task<Main> GetAsync(Guid code, string[] include = null)");
                stringBuilder.AppendLine($"        {{");
                ...
                stringBuilder.AppendLine($"            {{");
                stringBuilder.AppendLine($"                if (Guid.TryParse(term, out var code))");
                stringBuilder.AppendLine($"                {{");
                stringBuilder.AppendLine($"                    query = query.Where(c => c.Code.Equals(code));");
                stringBuilder.AppendLine($"                }}");
                stringBuilder.AppendLine($"            }}");
                stringBuilder.AppendLine($"");
